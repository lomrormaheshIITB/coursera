...
void erase(int x) {                   
  ...
  if(right != NULL and right->key == x){
    root = merge(left, merge(right->left, right->right));
  }

  // UPDATE SUM OF ROOT and ROOT->right
  update(root->right);
  update(root);
...
long long sum(int from, int to) {
  ...
  while(!window.empty()){
    ...
    window.pop();
    ans += temp->key;
    if(temp->left != NULL) window.push(temp->left);
    if(temp->right != NULL) window.push(temp->right);
  }
